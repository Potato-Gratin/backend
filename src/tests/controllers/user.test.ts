...
describe('UserController', () => {
  describe('createUser', () => {
    ...
    it('display_idが存在しない場合、400 Bad Requests を返すか', async () => {
      ...
      expect(jsonMock).toHaveBeenCalledWith({ message: 'displayId is required' });
    });
    ...
    it('display_id が重複している場合、409 Conflicted を返すか', async () => {
      (UserModel.create as jest.Mock).mockRejectedValue(new Error('displayId is conflicted'));

      await UserController.createUser(req as Request, res as Response);

      expect(statusMock).toHaveBeenCalledWith(409);
      expect(jsonMock).toHaveBeenCalledWith({ message: 'displayId is conflicted' });
    });

    it('サーバーエラーが発生した場合、500 Internal Server Error を返すか', async () => {
      (UserModel.create as jest.Mock).mockRejectedValue(new Error('some other error'));
